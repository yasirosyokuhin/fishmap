...
`public/game/` に、ブラウザだけで遊べる推理ロジックパズル（ロジックグリッドパズル）が入っています。手がかりをもとに「だれが・どの魚を・どこで釣ったか」を推理するゲームです。

- 開発サーバー起動中: `http://localhost:3000/game/index.html`
- デプロイ後: `https://<サイトのURL>/game/index.html`
- `public/game/index.html` をブラウザで直接開いても遊べます（ビルド不要）

むずかしさは3段階（3×3 / 4×4 / 5×5）。問題はアクセスのたびにランダム生成され、解が必ず1つに定まることをソルバーで保証しています。
